...
import { PrismaComboToDomainConverter } from "./converters/PrismaComboToDomainConverter";
...
export class PrismaComboRepository implements IComboRepository {
  ...
  async findById(id: string): Promise<Combo | null> {
    ...
    return PrismaComboToDomainConverter.convert(comboData, products);
  }
  ...
  async findManyByIds(ids: string[]): Promise<Combo[]> {
    ...
    const combos = combosData.map((c) =>
      PrismaComboToDomainConverter.convert(c)
    );
  ...
  async findMany({
    ...
    return new PaginationResponse<Combo>({
      data: data.map((c) => PrismaComboToDomainConverter.convert(c)),
      totalItems,
      currentPage: page,
      pageSize: size,
      totalPages,
  ...
  async create(combo: Combo): Promise<Combo> {
    ...
    return PrismaComboToDomainConverter.convert(comboData);
  }
  ...
  async update(combo: Combo): Promise<Combo> {
    ...
    return PrismaComboToDomainConverter.convert(
      updatedCombo,
